...
class CompaniesCreateAction extends CController {
    ...
    protected function checkInput(): bool {
        $fields = [
            ...
            'user' => 'required|not_empty|string',
            'password' => 'required|not_empty|string'
        ];
    ...
    protected function doAction(): void {
        ...
        try {
            ...
            if (!preg_match('/^[a-zA-Z0-9_\-]+$/', $name)) {
                throw new \Exception('Company Name must only contain alphanumeric characters, underscores or hyphens.');
            }
            
            // Check if group already exists
            $existing_group = API::HostGroup()->get([
                'filter' => ['name' => 'Tenant - ' . $name]
            ]);
            if ($existing_group) {
                throw new \Exception('Company/Tenant group already exists!');
            }
            
            // 1. Create Host Group
            $hg = API::HostGroup()->create(['name' => 'Tenant - ' . $name]);
            if (!$hg) {
                throw new \Exception('Failed to create host group.');
            }
            $group_id = $hg['groupids'][0];
            
            // 2. Create User Group with Read-Write permissions
            $ug = API::UserGroup()->create([
            ...
            // 4. Rebuild the MSP clients dashboard
